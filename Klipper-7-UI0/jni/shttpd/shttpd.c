...
//			LOGD("%s", "501 PUT Range Not Implemented");
...
//			LOGD("%s", "501 Bad method");
		...
		c->loc.chan.isGzFile = 0;
//		char gzipPath[10240] = {0};
//		sprintf(gzipPath, "%s.gz", path);
//		if ((c->loc.chan.fd = _shttpd_open(gzipPath,
//			O_RDONLY | O_BINARY, 0644)) != -1) {
//			c->loc.chan.isGzFile = 1;
//			sprintf(path, "%s", gzipPath);
//			LOGD("shttpd_decide_what_to_do! open path %s", path);
//			get_path_info(c, path, &st);
//			_shttpd_get_file(c, &st);
//		} else
		if ((c->loc.chan.fd = _shttpd_open(path,
			O_RDONLY | O_BINARY, 0644)) != -1) {
			//DBG(("shttpd_decide_what_to_do: open path %s", path));
			LOGD("shttpd_decide_what_to_do! open path %s", (path));
			_shttpd_get_file(c, &st);
		} else {
...
//		LOGD("%s", "501 Method Not Implemented");
...
static const struct opt {
	...
	{OPT_ROOT, "root", "\tWeb root directory", "/mnt/extsd/duetweb/", NULL},
