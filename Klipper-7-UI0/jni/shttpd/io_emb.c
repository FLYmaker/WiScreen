...
size_t
shttpd_printf_buffer(struct shttpd_arg *arg, const char *out_buffer, size_t length) {
	char		*buf = arg->out.buf + arg->out.num_bytes;
	int		buflen = arg->out.len - arg->out.num_bytes, len = 0;
	if (length > 0) {
		memcpy(buf, out_buffer, length);
		arg->out.num_bytes += length;
	}
...
void
shttpd_handle_default(struct shttpd_ctx *ctx,
		shttpd_callback_t func, void *data)
{
	...
	if ((e = malloc(sizeof(*e))) != NULL) {
		...
		e->callback_data = data;
		LL_TAIL(&ctx->default_handlers, &e->link);
	}
}
